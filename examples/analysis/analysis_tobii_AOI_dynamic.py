...
edfdata = read_tobii(fp, EDFSTART, stop=EDFSTOP, missing=INVALCODE, debug=False)
...
for trialnr in range(len(edfdata)):
	    # save pupil data
	out2.writerow([str(sys.argv[1]),imgname[trialnr],'Pupil-sizes',  trialnr, np.array_str(edfdata[trialnr]['size'],max_line_width=np.inf) ])
	out2.writerow([str(sys.argv[1]),imgname[trialnr],'Timestamp',  trialnr, np.array_str(edfdata[trialnr]['time'],max_line_width=np.inf) ])	
