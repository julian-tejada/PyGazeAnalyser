	...
	for i in range(20,len(raw)):
		...
		if started:
			...
			# check for new start otherwise
				...
				if start in line[1] or i == len(raw)-1:
					started = True
					trialend = True
			...
			if trialend:
				...
				trial['events']['Sfix'], trial['events']['Efix'] = fixation_detection(trial['x'],trial['y'],trial['trackertime'],missing=missing, mindur=50, maxdist=40)
				trial['events']['Ssac'], trial['events']['Esac'] = saccade_detection(trial['x'],trial['y'],trial['trackertime'],missing=missing,minlen=5, maxvel=40, maxacc=340)
				# add trial to data
				data.append(trial)
				# reset stuff
				x = []
		...
		if started:
			...
			# 501.8202, 546.3453, 609.3405, 623.2287, 16.7916, 0.5539, 0.5209'
				...
				try:
					...
					# contain valid data
					elif line[10] != "NA"  and line[12] != "NA" :
						if float(line[10]) == 0:
							val = float(line[12])
						elif float(line[12]) == 0:
							val = float(line[10])
						else:
							val = (float(line[10]) + float(line[12])) / 2.0
						size.append(val)
					if 	line[8] != "NA"  and line[9] != "NA" :
